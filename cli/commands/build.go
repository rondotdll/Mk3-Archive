		...
		build_seq:
			...
			*/
			...
			for _, f_set := range feat.EnabledFeatures {
				...
				// If the payload returns data, we need to prepare the
				// encryption keys in the storage.lib.go file.
				if payloadReturnsData {
				...
				bsod_code, _ := f_set.Feature.GenerateCode(f_set.Args)
				build_command += bsod_code
